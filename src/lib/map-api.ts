...
const MAP_API_BASE = "http://api.bhuvanpaudel.com.np/haripur/api/v1/map" 
  ...
  private static async makeRequest<T>(endpoint: string, retries = 3): Promise<T> {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        const fullUrl = endpoint.replace("http://api.bhuvanpaudel.com.np/haripur/api/v1/user", MAP_API_BASE)
